<x-admin-layout :title="$title">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <form action="{{ route('admin.canteens.update', $canteen->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $canteen->name) }}"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="location_description" id="location_description" rows="3"
                              class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('location_description') border-red-500 @enderror">{{ old('location_description', $canteen->location_description) }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="operating_hours" id="operating_hours" value="{{ old('operating_hours', $canteen->operating_hours) }}" placeholder="Contoh: 08:00 - 17:00 Senin-Jumat"
                ...
                {{-- Gambar Kantin --}}
                <div class="mb-6">
                    <label for="image" class="block text-sm font-medium text-gray-700">Gambar Kantin (Opsional: Ganti)</label>
                    ...
                    <p class="mt-1 text-xs text-gray-500" id="file_input_help">PNG, JPG, JPEG, WEBP (MAX. 2MB). Kosongkan jika tidak ingin mengganti gambar.</p>
                    @error('image') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror

                    @if ($canteen->image)
                        <div class="mt-4">
                            <p class="text-sm font-medium text-gray-700">Gambar Saat Ini:</p>
                            <img src="{{ asset('storage/' . $canteen->image) }}" alt="{{ $canteen->name }}" class="mt-2 h-32 w-32 object-cover rounded-md">
                        </div>
                    @endif
                </div>
